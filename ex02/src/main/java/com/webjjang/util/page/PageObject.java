...
public class PageObject {
	...
	//하단 부분의 페이지 링크 부분(페이지네이션)에 나타날 페이지의 갯수 -> perGroupPageNum
	private int perGroupPageNum;
	//페이지 그룹에 나타나는 처음 페이지 - 현채 페이지에 따라 다르게 설정된다.
	//totalPage가 나오면 꼐산할수 있다. - totalPage가 25면 endPage가 30으로 셋팅되는 경우
    //                          -endPage가 totalPage를 넘는 경우는 endPage로 
...
//		this.endRow = page * perPageNum; // 위에 문장과 같다.
	...
	// 생성자에서 데이터를 처리하도록 작성한다.- > spring 에서는 setter 를 만들어서 해야한다.
...
//	}
	...
	//전체 페잊가 구해지면 리스트 화면 하단부분 페이지 네이션 시작 페이지와 끝 페이지를 계산할수 있다.
	// servuce에서 계산하는것으로 한다.
