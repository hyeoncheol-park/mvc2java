...
public class NoticeDAO {
	...
	//servic.NoticeListServic -> dao.NoticeDAO
	// 1. 공지사항 리스 데이터 가져오기
...
//				2 정렬해서 가져온 데이토 순서번호를 붙인다.
				sql = "select rownum rnum,no,title,startDate,endDate,writeDate,updateDate"
						+ " from(" + sql +")";
//				3 순서번호를 붙인 데이터에서 페이지에 맞는 데이터를 찾아온다.
				sql="select * "
						+" from( " +sql +")"
		...
		System.out.println("NoticeDAO.list():" + list);
		return list;
	} //end of list()
		
	// service.NoticeViewService -> dao.NoticeDAO
		// 2. 공지사항 갯수  데이터 가져오기 - 한개 데이터 - 글번호에 따라서 결정(전달 받는다.)
	...
	public int getTotalRow() throws Exception{
		...
		System.out.println("NoticeDAO.getTotalRow(0.totalRow"+totalRow);
			...
			int a=pstmt.executeUpdate();
			 ...
			 if(a > 0 )
					System.out.println("글수정 성공");
						...
						System.out.println("글수정이 되지 않았습니다. -글번호를 확인해주세요");
			// 6. 표시 / 저장
		...
		// service.NoticeUpdateService -> dao.NoticeDAO
		// 5. 공지사항 글삭제 글번호 데이터 가져오기 - 글번호 전달 받아서 DB에 수정처리
		//  여기서는 예외처리하지 않고 throw 시키는 것으로 할 수 있다.
		//  NoticeController : 실행내용 결정 - 데이터 수집 / 데이터 표시
		//  NoticeController -> Service -> DAO
		public void delete(int no) throws Exception{
			...
			// 확인해야할 데이터 - 번호,
			System.out.println("NoticeDAO.delete().dto:" + no);
