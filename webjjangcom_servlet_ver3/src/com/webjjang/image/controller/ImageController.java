...
public class ImageController implements Controller {
	...
	public String execute(HttpServletRequest request,HttpServletResponse response,
		...
		case "/image/view.do":
			...
			// Execite.service(qnaViewService, 글번호)
...
//				getDTO(multi.getParameter("title"),
...
//					+multi.getParameter("perPageNum");
		...
		case "/image/update.do":
			...
			Execute.service(updateService, 
					dto);
			// 전달된 첨부 파일이 있는 경우는 이미지 교체를 해야 하므로 원래 있었던 파일은 지운다.
	...
	private ImageDTO getDTO(HttpServletRequest request) throws Exception {
		...
		// 저장된 파일사이즈를 줄여서 리스트 표시하기 위한 파일로 복사 -> dog01.jpa -> s_dog01.jgp
		// webjjangma 카페서 업로드 검색 리사이징 프로그램 참조 + 라이브러리 등록 
		//ImageResizing.imageResizing(realPath,이미지 파일명,앞첨자,너비,높이)
		//첨부파일 있는 경우만 처리를 한다. 첨부가 없는 경우 fileName
		if(fileName != null && !fileName.equals("") 
				&& !fileName.equals(path+"null"))
